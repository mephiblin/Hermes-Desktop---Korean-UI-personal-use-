...
import { koreanSlashDescription, koreanSlashGroup } from '@/i18n/ko-slash-commands'
...
export function useSlashCompletions(options: {
  ...
  const toItem = useCallback((entry: CompletionEntry, index: number): Unstable_TriggerItem => {
    ...
    const rawMeta = textValue(entry.meta)
    const meta =
      locale === 'ko' && !isDesktopSlashExtensionCommand(command)
        ? koreanSlashDescription(command, rawMeta)
        : rawMeta
    const group = textValue(entry.group)

    const metadata: SlashItemMetadata = {
      command,
      display,
      meta,
      group: locale === 'ko' ? koreanSlashGroup(group) : group,
