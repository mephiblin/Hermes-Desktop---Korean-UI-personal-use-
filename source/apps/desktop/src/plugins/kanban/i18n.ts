...
const ko: KanbanMessages = {
  ...en,
  introBody:
    '카드는 사용자가 직접 실행하는 것이 아니라 에이전트가 실행합니다. 담당자가 지정된 카드를 준비 상태에 놓으면 1분 안에 에이전트가 가져갑니다. 담당자가 없으면 실행되지 않습니다. 분류: 에이전트가 아이디어를 먼저 제대로 된 작업으로 구체화합니다. 할 일: 다른 카드가 끝나기를 기다립니다. 예약됨: 설정된 시간을 기다립니다. 실행 중과 검토: 에이전트가 처리하는 단계이므로 건드리지 마세요. 차단됨: 사용자의 응답을 기다리는 상태입니다. 작업 결과는 카드에 표시됩니다.',
  introGotIt: '확인'
}
